...
def test_min_frequency_int_threshold_collapses_rare():
    ...
    df = pd.DataFrame({"country": ["A", "A", "A", "A", "B", "B", "B", "C", "D", "E"]})
    transformed, msgs = apply_preprocessing(
        df,
...
def test_min_frequency_and_group_by_frequency_compose():
    ...
    df = pd.DataFrame({"col": ["A", "A", "A", "B", "B", "C", "D", "E"]})
...
def test_bucketing_assigns_interval_labels_by_default():
    ...
    transformed, msgs = apply_preprocessing(
        df,
        {"age": {"bucketing": {"edges": [0, 18, 40, 65, 100]}}},
    )
    # pandas represents the first bin as (-0.001, 18.0] when include_lowest=True
...
def test_bucketing_uses_custom_labels():
    df = pd.DataFrame({"age": [5, 20, 35, 55, 80]})
    transformed, msgs = apply_preprocessing(
        df,
        {
...
def test_bucketing_requires_edges_key():
    df = pd.DataFrame({"age": [10, 20, 30]})
    with pytest.raises(ValueError, match="requires an 'edges' key"):
        apply_preprocessing(df, {"age": {"bucketing": {}}})


def test_bucketing_requires_at_least_two_edges():
    df = pd.DataFrame({"age": [10, 20, 30]})
    with pytest.raises(ValueError, match="at least 2 values"):
        apply_preprocessing(df, {"age": {"bucketing": {"edges": [0]}}})


def test_bucketing_rejects_mismatched_labels_length():
    df = pd.DataFrame({"age": [10, 20, 30]})
    with pytest.raises(ValueError, match="list of 2 strings"):
        apply_preprocessing(
            df,
            {"age": {"bucketing": {"edges": [0, 18, 100], "labels": ["Only One"]}}},
        )


def test_bucketing_values_outside_edges_become_nan():
    df = pd.DataFrame({"score": [5, 50, 150]})
    transformed, _ = apply_preprocessing(
        df,
        {"score": {"bucketing": {"edges": [0, 25, 100]}}},
    )
    assert pd.isna(transformed["score"].iloc[2])
