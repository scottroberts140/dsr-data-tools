...
* **Derived Flag Recommendations**: Added `NonZeroFlagRecommendation` and `StringMatchFlagRecommendation` for creating derived presence/match flag columns from numeric and string source data.

### Changed

* **RecommendationManager Support**: Updated staged execution, summary output, cleanup behavior, and YAML loading to understand the new derived flag recommendations while keeping the source column intact as the input feature.

### Fixed

* **YAML and Snippet Alignment**: Updated recommendation YAML snippets and deserialization handling so the new flag recommendations round-trip cleanly with the existing `Recommendation` model pattern.
